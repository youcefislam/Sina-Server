create schema if not exists sinaV1;
...
Create table IF NOT EXISTS Daira (
    ...
    idWilaya INT,
    primary key (idDaira),
    Constraint FK_daira_IdWil FOREIGN KEY (idWilaya) references Wilaya(idWilaya) ON DELETE CASCADE
);
...
Create table IF NOT EXISTS Commune (
    ...
    Constraint FK_com_IdDa FOREIGN KEY (idDaira) references Daira (idDaira) ON DELETE CASCADE
);
...
Create table IF NOT EXISTS TypeMaladie (
...
Create table IF NOT EXISTS Medecin(
    ...
    Constraint Fk_Med_IdDaira FOREIGN KEY (idDaira) references Daira(idDaira) ON DELETE
    SET
        NULL,
...
Create table IF NOT EXISTS Patient (
    ...
    Constraint FK_pat_PatTypeMal FOREIGN KEY (idTypeMaladie) references TypeMaladie(idTypeMaladie) ON DELETE
    SET
        NULL,
        Constraint FK_pat_idCommune FOREIGN KEY (idCommune) references Commune(idCommune) ON DELETE
    SET
        NULL,
        Constraint FK_pat_idMed FOREIGN KEY (idMedecin) references Medecin (idMedecin) ON DELETE
    SET
        NULL,
        Constraint FK_pat_idProche FOREIGN KEY (idProche) references Proche (idProche) ON DELETE
    SET
        NULL,
...
Create table IF NOT EXISTS FichierECG (
...
Create table IF NOT EXISTS ListAtt(
...
Create table IF NOT EXISTS ListMedicament (
    dateDebut DATETIME,
    posologie INT,
    idPatient INT NOT NULL,
...
Create table IF NOT EXISTS NoteMedecin (
...
CREATE TABLE IF NOT EXISTS patientNonVerifie(
    idPatient INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS medecinNonVerifie(
